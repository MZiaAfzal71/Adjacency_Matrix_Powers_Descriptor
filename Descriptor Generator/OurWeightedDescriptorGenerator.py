...
def smiles_to_weighted_adjacency_matrix(smiles):
    ...
    for bond in mol.GetBonds():
        ...
        bond_weight = bond.GetBondTypeAsDouble()
        A[i, j] = bond_weight
        A[j, i] = bond_weight  # symmetric
...
def process_excel_file(input_file, output_file, n):
    ...
    for index, row in tqdm(df.iterrows(), total = len(df)):
        smiles = row['SMILES']
        A, atomic_numbers = smiles_to_weighted_adjacency_matrix(smiles)
...
output_file = "Excel Files/OurDescriptorWeighted.xlsx"
